...
@Component
public class CustomerMessageStrategy implements RoleMessageStrategy {

	private static final Logger log = LoggerFactory.getLogger(CustomerMessageStrategy.class);
	private final RabbitTemplate rabbitTemplate;

	public CustomerMessageStrategy(RabbitTemplate rabbitTemplate) {
	...
	public void handlePreSend(StompHeaderAccessor accessor) {
		...
		try {
			...
			log.warn("Failed to publish customer preSend event", ex);
		}
	}
	...
	public void handlePostSend(StompHeaderAccessor accessor) {
		...
		try {
			...
			log.warn("Failed to publish customer postSend event", ex);
		}
	}
}
